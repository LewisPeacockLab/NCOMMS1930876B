function[] = grp_analysis_timecourse_workingmemory_sorted(args, grp_mvpaout, dirs)
...
if base_corr
    ...
    base_name = sprintf('uncorr_%s_n%s', args.analysis_basename, num2str(length(xsub_groups)));
end
...
for xcond = all_conds
    ...
    if base_corr
        ...
        for xtr = 1:baseline_trs
            ...
            for it = 1:length(xsub_groups)
                xsub = xsub_groups(it);
                xbase(it) = mean(grp_mvpaout{xsub}.decode.timecourse.condition{xcond}.target{xtarg}.evidence{xtr});
            end
    ...
    for xtr = 1:n_tc_trs
        ...
        for it = 1:length(xsub_groups)
            xsub = xsub_groups(it);
            tevidence(it) = mean(grp_mvpaout{xsub}.decode.timecourse.condition{xcond}.target{xtarg}.evidence{xtr});
        end
...
for xsubplt = [1 3:5]
    ...
    title(sprintf('%s classifier: targets (N=%s, FDR alpha=%1.2f, TR/blk=%s)', ...
...
fig_fname = fullfile(dirs.mvpa.group.out{xph}, ...
    sprintf('tc_targets_blk%d_%s', n_tr_blks, base_name));
...
%*************** save fig
fig_fname = fullfile(dirs.mvpa.group.out{xph}, ...
    sprintf('tc_cond_blk%d_%s', n_tr_blks, base_name));
savefig(xfig, sprintf('%s.fig', fig_fname));
set(gcf,'PaperUnits','inches','PaperPosition', fig_rect/100)
print('-dpng', '-r300', sprintf('%s.png',fig_fname))
saveas(xfig, sprintf('%s.png', fig_fname), 'png')
