function[mvpaout] = parse_mvpa_results_operation_new(args, ph, dirs, check_perf)
...
n_vols   = 0;
for xiter = 1:n_iter
    n_vols = n_vols + size(ph.results.iterations(xiter).acts, 2);
end
...
for xcond = 1:n_target
    if xcond == 2
        xunit = getDATA(xmatrix', xheader, {'operation','selected'}, {xcond, 1});
    else
        xunit = getDATA(xmatrix', xheader, {'operation'}, {xcond});
    end
...
xcond_name   = xparam.operation_names;
...
if check_perf% check mvpa performance
    ...
    for xrun = 1:n_runs
        for xtrial = 1:n_trials(xrun)
            ...
            xcond    = unique(xmatrix(findCol(xheader, {'operation'}), xunit));
