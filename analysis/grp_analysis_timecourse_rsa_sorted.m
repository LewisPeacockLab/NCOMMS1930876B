function[grp_rsa] = grp_analysis_timecourse_rsa_sorted(grp_rsa, args, dirs)
...
for xcond = all_conds
    ...
    for xtr = 1:baseline_trs
        xmean_base(xtr) = mean(grp_rsa{xph}.timecourse.cond{xcond}.targ{xtarg}.tr{xtr}.corr_w);
    end
    
    for xtr = 1:n_tc_trs
        %*************** baseline corrected
        xevidence = grp_rsa{xph}.timecourse.cond{xcond}.targ{xtarg}.tr{xtr}.corr_w - mean(xmean_base);
...
fig_fname = fullfile(output_dir, ...
    sprintf('tc_targets_blk%d_%s_n%s', n_tr_blks, basename, num2str(length(xsub_groups))));
...
%*************** save fig
fig_fname = fullfile(output_dir, ...
    sprintf('tc_cond_blk%d_%s_n%s', n_tr_blks, basename, num2str(length(xsub_groups))));
savefig(xfig, sprintf('%s.fig', fig_fname));
set(gcf,'PaperUnits','inches','PaperPosition', fig_rect/100)
print('-dpng', '-r300', sprintf('%s.png',fig_fname))
saveas(xfig, sprintf('%s.png', fig_fname), 'png')
