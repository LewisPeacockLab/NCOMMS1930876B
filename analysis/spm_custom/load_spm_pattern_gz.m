function [subj] = load_spm_pattern_gz(subj,new_patname,maskname,filenames,varargin)
...
for h=1:nFiles
    [V_gz{h}, ~, vol_gz{h}] = icatb_read_gzip_nii(filenames{h}); % <--------
end
...
subj = set_objsubfield(subj,'pattern',new_patname,'header', ...
			 'vol',vol_gz,'ignore_absence',true);
