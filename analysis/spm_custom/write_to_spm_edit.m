function [save, masked_pattern_matrix] = write_to_spm_edit(subj,objtype,objin,varargin)
...
for cur_object_index=1:num_in_group
    ...
    if ~(strcmp(pathname,''))
        ...
        if (strcmp(output_filename,''))
            final_path = [pathname '/' obj_name{1}];
        else
    ...
    switch lower(objtype)
        ...
        case {'pattern'}
            ...
            if (~new_header)
                cur_vol = get_objsubfield(subj, objtype, cur_obj_name,'header','vol');
%                 cur_vol{1}(1).fname=final_path;
                if ~(strcmp(final_path,''))
                    for fname_index=2:pattern_size(4)
%                         cur_vol{1}(fname_index).fname=final_path;
                        cur_vol{1}(fname_index) = cur_vol{1}(1);
