...
if sum(localizer_classification)
    ...
    if sum(localizer_classification(1:3))
        for xsub = args.filtered_subs
            ...
            if localizer_classification(1), localizer_classification_01(args, dirs); end
            if localizer_classification(2), localizer_classification_02(args, dirs); end
            if localizer_classification(3), localizer_classification_03(args, dirs); end
        end
    end
    ...
    if sum(localizer_classification(4:5))
        ...
        if localizer_classification(4)
            ...
            %*************** group analysis
            localizer_classification_04(args, dirs);
        end
    end
end
...
if sum(mvpa_decode)
    ...
    if sum(mvpa_decode(1:3))
        for xsub = args.filtered_subs
            ...
            %*************** classification
            if mvpa_decode(1), mvpa_decode_01(args, dirs); end
            if mvpa_decode(2), mvpa_decode_02(args, dirs); end
            if mvpa_decode(3), mvpa_decode_03(args, dirs); end
        end
    end
...
if sum(operation_mvpa_decode)
    ...
    if sum(operation_mvpa_decode(1:3))
        for xsub = args.filtered_subs%args.g_sub
            ...
            %*************** classification
            if operation_mvpa_decode(1), mvpa_operation_01(args, dirs); end
            if operation_mvpa_decode(2), mvpa_operation_02(args, dirs); end
            if operation_mvpa_decode(3), mvpa_operation_03(args, dirs); end
        end
    end
        ...
        if operation_mvpa_decode(6)
            ...
            if args.subj_impmap{xphase}
                for xsub = args.filtered_subs
                    ...
                    operation_importance_map_subj(args, dirs);
                end
            end
            ...
            operation_importance_map(args, dirs);
        end
    end
end
...
if sum(rsa_decode)
    ...
    elseif rsa_decode(2)
        for xsub = args.filtered_subs
            ...
            if ~strcmp(args.level, 'item')
                rsa_decode_02(args, dirs);
            else
                rsa_decode_02_item(args, dirs);
            end
    ...
    if sum(rsa_decode(3:4))
        ...
        if rsa_decode(3)
            if ~strcmp(args.level, 'item')
                rsa_decode_03(args, dirs);
            else
                rsa_decode_03_item(args, dirs);
            end
            
        elseif rsa_decode(4) && (strcmp(args.level, 'item'))
            rsa_decode_04_bootstrap(args, dirs)
        end
    end
end
...
if sum(operation_mvpa_stw)
    ...
    if sum(operation_mvpa_stw(1:2))
        for xsub = args.filtered_subs
            ...
            %*************** classification
            if operation_mvpa_stw(1), stw_operation_01(args, dirs); end
            if operation_mvpa_stw(2), stw_operation_02(args, dirs); end
        end
    end
