function[grp_rsa] = grp_analysis_timecourse_rsa_sorted_selected_cate(grp_rsa, args, dirs)
...
xalpha          = args.alpha;
baseline_trs    = 6;%args.baseline_trs;
...
for xcond = all_conds
    ...
    for xtr = 1:baseline_trs
        for xsubcate = 1:3
            xmean_base(xtr) = ...
                mean(grp_rsa{xph}.timecourse.cond{xcond}.cate{select_category}.subcate{xsubcate}.targ{xtarg}.tr{xtr}.corr_w);
        end
    end
    ...
    for xtr = 1:n_tc_trs
        ...
        t_evidence = [];
        for xsubcate = 1:3
            t_evidence = vertcat(t_evidence, ...
                grp_rsa{xph}.timecourse.cond{xcond}.cate{select_category}.subcate{xsubcate}.targ{xtarg}.tr{xtr}.corr_w);
        end
        xevidence = mean(t_evidence) - mean(xmean_base);
        
        tc.evidence{xcond}.tr(:, xtr) = xevidence;
        tc.mean(xcond, xtr)           = mean(xevidence);
        tc.se(xcond, xtr)             = std(xevidence)/sqrt(length(xsub_groups));
        tc.ci(xcond, xtr)             = (std(xevidence)/sqrt(length(xsub_groups))) * 1.96;
    end
end
...
for xcond = it_conds
    for xtr = 1:n_tc_trs
        ...
        tc_diff.ci(xcond, xtr)             = (std(xevidence)/sqrt(length(xsub_groups))) * 1.96;
    end
end
...
for xsubplt = [1 3:5]
    ...
    [xh, xcrit_p, xadj_ci_cvrg, xadj_p] = fdr_bh(pvalue_matrix, xalpha, 'pdep', 'no');
     ...
     for xblk = 1:n_stat_blks
         ...
         if xh(xblk)
             ...
             text(xx, y_lim(2) - 0.02, '*', 'FontSize', 20);
             h = text(xx, y_lim(1) + 0.02, sprintf('%s\nadj: p=%s', xstats, num2str(xadj_p(xblk))), 'FontSize', 8);
             set(h, 'Rotation', 90);
         else
             h = text(xx, y_lim(1) + 0.02, sprintf('adj: p=%s', num2str(xadj_p(xblk))), 'FontSize', 8);
             set(h, 'Rotation', 90);
         end
     end
    ...
    title(sprintf('%s RSA: targets (N=%s, FDR alpha=%1.2f, TR/blk=%s)', ...
        args.level, num2str(length(xsub_groups)), xalpha, num2str(n_tr_blks)));
    xlabel('Volume (tr)');
...
%*************** save fig
fig_fname = fullfile(output_dir, ...
    sprintf('tc_cate%d_targets_blk%d_%s_n%s', select_category, n_tr_blks, basename, num2str(length(xsub_groups))));
...
for xsubplt = 1:length(it_conds)
    ...
    title(sprintf('%s classifier: %s (N=%s, FDR alpha=%1.2f, TR/blk=%s)', ...
        args.level, xlegend, num2str(length(xsub_groups)), xalpha, num2str(n_tr_blks)));
    xlabel('Volume (tr)');
    ylabel('classifier evidence (target)');
...
%*************** save fig
fig_fname = fullfile(output_dir, ...
    sprintf('tc_cond_cate%d_blk%d_%s_n%s', select_category, n_tr_blks, basename, num2str(length(xsub_groups))));
savefig(xfig, sprintf('%s.fig', fig_fname));
set(gcf,'PaperUnits','inches','PaperPosition', fig_rect/100)
print('-dpng', '-r300', sprintf('%s.png',fig_fname))
saveas(xfig, sprintf('%s.png', fig_fname), 'png')
